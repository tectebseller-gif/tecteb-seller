...
۲) بازگشت به نسخه قبلی حالا خودکار امن است. کافی است افزونه را غیرفعال کنید:
   ...
   بازگشت مستلزم «از سرگیری فروش» نیست: «تلاش دوباره توقف» و «بازگرداندن
   سفارش‌های نگه‌داشته» دو دکمه جدا هستند و هیچ‌کدام خرید را باز نمی‌کنند.
...
۲-الف) سفارش پرداخت‌نشده‌ای که قلم بازارگاه دارد، هنگام توقف به وضعیت on-hold
   ...
   وضعیت قبلی‌اش نگه داشته می‌شود. اثر جانبی‌اش را بدانید: WooCommerce با
   همین وضعیت موجودی را کم و هنگام بازگرداندن زیاد می‌کند.
...
=======================
  ...
  - پیوست فایل در تیکت، اعلان‌ها، نظرات، گزارش‌ها و SEO خودکار — ساخته نشده‌اند
