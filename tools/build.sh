...
۲) بازگشت به نسخه قبلی یک استثنا دارد. محصولی که این نسخه به WooCommerce
   فرستاده، یک پست منتشرشده WooCommerce است و با بازگشت پاک نمی‌شود — ولی
   نسخه قبلی نه گاردِ خرید دارد و نه ثبت سفارش، پس آن محصول‌ها قابل خرید
   می‌مانند و هیچ سهم مالی و هیچ خط دفترکلی برایشان نوشته نمی‌شود.
   ترتیب درست: اول در بازارگاه محصول‌ها یا فروشنده‌ها را تعلیق کنید (خود این
   نسخه آن‌ها را draft و از فروشگاه خارج می‌کند)، بعد ZIP قبلی را بازگردانید.
   هر دو مسیر اندازه‌گیری شده: docs/upgrade-and-rollback.md بند ۵٫۱
...
=======================
  ...
  - checkout بلوکی WooCommerce (Store API)؛ آزمون روی checkout کلاسیک بود
  - رفتار با دکانِ واقعاً نصب‌شده؛ در محیط آزمون، محصول شاهد با متای
    _dokan_vendor_id ساخته شد که «مال ما نیست» را می‌سنجد، نه تداخل افزونه‌ای
