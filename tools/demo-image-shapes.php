<?php
/**
 * The rasteriser, with no WordPress in it.
 *
 * Split out of `demo-image.php` so the same drawing code runs two ways:
 * inside WordPress, where the caller attaches the result to a post, and from
 * a plain CLI, where it writes PNG files that a browser then uploads through
 * the product form like any other picture. One implementation, two callers —
 * the alternative was a second copy of a rasteriser, which is a second thing
 * to be wrong.
 *
 * Pure arithmetic: this host's PHP has neither GD nor Imagick, so a PNG is
 * assembled here as a signature and three chunks, with `gzcompress` for the
 * zlib stream.
 */
