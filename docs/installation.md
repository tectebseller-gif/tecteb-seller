...
> **از ۱۱ سپتامبر:** گیت‌های G-06 و G-07 به `tools/acceptance-gates.sh` افزوده
> شدند و دیگر دستی اجرا نمی‌شوند. اسکریپت G-07 را پیش از G-08 و G-06 را در
> **انتهای** اجرا می‌گذارد (چون افزونه را حذف می‌کند) و بعد دوباره نصبش می‌کند.
> `compatibility-mode` زیرفرمان می‌گیرد نه فلگ: `wp wc hpos compatibility-mode
> enable|disable`؛ شکل `--enable` بی‌صدا بی‌اثر است و هر دو حالت را یکسان
> می‌کند. نام فایل بسته هم از این پس نسخه‌دار است:
> `tecteb-marketplace-core-<version>.zip` (دفتر تصمیم‌ها، F-06).
...
این کار سطر «اجرای واقعی روی ۸٫۱» را از `Not Run` خارج می‌کند. **انجام شد:**
PHP 8.1.32 از سورس ساخته شد و کل پروتکل روی آن اجرا شد
(`docs/evidence/acceptance/php81/`). آنچه باقی است، ۸٫۱٫۳۴ دقیقِ سایت است. نتیجه روی
