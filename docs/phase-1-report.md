...
| ۸ | `8a838e3c…61ecba` (تا `alpha.15` اینجا `…5a68a4` نوشته شده بود — غلط، و با هش کاملِ همین سند در جدول بالا در تناقض) | نسخه `0.1.0-alpha.3`: **بخش فروشندگان** (درخواست، مدارک پویا، بررسی مدیر، پیشخوان `/vendor/`) | تحویل شد — و یک نقص بسته‌بندی داشت: `assets/vendor/tmc-vendor.css` داخلش نبود، پس ناحیه فروشنده بدون استایل بالا می‌آمد |
| ۹ | `79df5ff6…8b4c100d` | نسخه `0.1.0-alpha.4`: مقدار پیام‌های خطا پس از redirect، مدارک بیرون از ریشه‌های وب (ADR-007)، و اصلاح نقص بسته‌بندی | تحویل شد |
| ۱۰ | `69e8292f…1ded465c` | نسخه `0.1.0-alpha.5`: تنظیمات فروشگاه، پرسنل و نقش‌ها، دعوت و دسترسی‌ها (`docs/phase-3-vendor-completion.md`)، و اصلاح تشخیص محل مدارک در نصب تو‌در‌تو | تحویل شد |
| ۱۱ | `dd8e364f…c622a385` | نسخه `0.1.0-alpha.6`: **بخش محصولات** (مالکیت، تأیید انتشار، نسخهٔ پیشنهادی، موجودی، تصویر، فیلدهای پویای دسته، CSV)، تعلیق و بازگردانی فروشنده، موتور کمیسیون و دفترکل، و دروازهٔ عملیات سفارش (`docs/phase-4-products.md`) | تحویل شد |
| ۱۲ | `41c30c94…363a06b2` | نسخه `0.1.0-alpha.7`: **اتصال به ووکامرس** (ADR-008: مرجع هر فیلد، پیوند دوطرفه، بدون محصول تکراری)، **تنوع کامل محصول متغیر**، **ثبت سفارش به تفکیک فروشنده** با دفترکل، صفحهٔ `/vendor/orders/`، توقف خرید در تعلیق/ناموجودی/قفل مالی بدون دست‌زدن به محصولات سایت و دکان، SEO فقط‌مدیر، جست‌وجوی فهرست و مرتب‌سازی گالری (`docs/phase-5-catalog-and-orders.md`) | **بسته فعلی** |
| ۱۳ | **`ab2a0e98…d68783da`** | نسخه `0.1.0-alpha.8`: **بازگشت امن** (غیرفعال‌سازی محصولات را از فروش خارج می‌کند، فعال‌سازی دوباره خودکار برنمی‌گرداند، دکمهٔ توقف/از سرگیری برای مدیر)، **سه در دیگر خرید** (سبد از قبل پُرشده، Store API و سبد بلوکی، لینک پرداخت سفارش پرداخت‌نشده)، **هم‌زیستی با دکانِ واقعاً فعال**، **تسویه و برداشت** (FIN-05 کامل، دفترکل متوازن، بدون retry خودکار)، و پیام کوتاه خریدار جدا از جزئیات مدیر (`docs/phase-6-safe-stop-and-settlement.md`) | **بسته فعلی** |

هش کامل بسته فعلی:
`ab2a0e98c5cf1e4fd11e3d80a53d580328806b61722c4388cccc1961d68783da`
نام فایل: `tecteb-marketplace-core-0.1.0-alpha.8.zip`

> **یک ابزار آزمون که دروغ می‌گفت.** بررسی «commission preserved» در
> `tools/upgrade-rollback-check.sh` مقدار را در عمق اشتباهِ ساختار option
> می‌نوشت (`tmc_settings.x` به‌جای `tmc_settings.values.x`) و grepِ assertion
> مقدارِ درستِ تودرتو را می‌دید — مقداری که **اجرای قبلی** جا گذاشته بود. پس
> این بررسی سال‌ها می‌توانست قبول شود بی‌آنکه چیزی را بسنجد. با ریست‌شدن
> تنظیمات در این دور آشکار شد. حالا seed و assertion به یک جا اشاره می‌کنند.

> **`php -l` روی یک نسخه کافی نیست.** بستهٔ ۱۲ در ساخت اولش خطای نحویِ صفری
> داشت و روی PHP 8.1 **هر صفحهٔ سایت را می‌کشت**: `EnumCase->value` داخل یک
> `const` از ۸٫۲ مجاز است و روی ۸٫۱ فاتالِ زمانِ **کامپایل** می‌دهد، پس نه یک
> صفحه بلکه کل سایت می‌مرد. ماشین ساخت ۸٫۴ است و lint پاک گزارش می‌داد.
> `tools/lint.sh` حالا روی **هر نسخهٔ PHP که افزونه ادعای اجرا رویش دارد**
> اجرا می‌شود (۸٫۴ و ۸٫۱)، و همان اجرا خطا را گرفت. PHPCompatibility کنارش
> می‌ماند ولی این دسته را نمی‌گیرد.

> **نقص بستهٔ ۸ چطور از همه آزمون‌ها رد شد.** ابزار ساخت هر پوشه‌ای به نام
> `vendor` را در هر عمقی حذف می‌کرد — قاعده‌ای که برای پوشهٔ Composer نوشته
> شده بود — و `assets/vendor/` قربانی نامش شد. آزمون بسته‌بندی هم همان قاعده
> را داشت، پس با ساخت هم‌نظر بود و چیزی نمی‌دید؛ آزمون‌های مرورگری روی نسخه‌ای
> اجرا می‌شدند که فایل‌ها را از سورس داشت، نه از ZIP. حالا حذف فقط در ریشهٔ
> payload انجام می‌شود، و آزمونی اضافه شده که **هر فایل `assets/` در درخت کاری
> را در ZIP الزام می‌کند**؛ روی بستهٔ معیوب شکست می‌خورد (بررسی‌شده) و روی
> بستهٔ فعلی قبول می‌شود.
