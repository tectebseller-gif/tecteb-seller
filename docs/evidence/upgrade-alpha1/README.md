# شواهد تمرین ارتقا و بازگشت — از `0.1.0-alpha.1` که روی staging نصب است
...
بستهٔ مبدأ همان فایلی است که مالک نصب کرده — sha256 `c37f8902…7f46f1`،
بازیابی‌شده از commit `994b465` — نه یک ساخت تازه با همان شماره نسخه.
محیط و sha256 هر دو بسته: `00-environment.txt`.

| فایل | چیست |
|---|---|
| `01-before-upgrade.txt` | وضعیت کامل سایت روی `alpha.1` با schema ۱ |
| `02-after-upgrade.txt` | همان وضعیت پس از ارتقا به `alpha.4` |
...
| `06-after-rollback.txt` · `07-diff-rollback.txt` | پس از بازگشت به `alpha.1` |
