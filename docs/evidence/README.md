...
> **از `0.1.0-alpha.14`، تصویرها در آرشیو جداگانه‌اند.**
>
> هیچ تصویری حذف نشده و همه در همین مخزن سر جایشان هستند. ولی در بستهٔ تحویل
> دو آرشیو وجود دارد:
>
> | آرشیو | چه دارد | چرا |
> |---|---|---|
> | `…-source-<version>.tar.gz` | کد، آزمون‌ها، اسناد، و **نیمهٔ متنی همین پوشه** (`.txt`، `.json`، `.log`، `.html`، `.md`) | همان نیمه‌ای که یک بازبین می‌خواند |
> | `…-evidence-<version>.tar.gz` | فقط تصویرهای `docs/evidence/**` (۲۳۷ فایل) | ۴۹ مگابایت از ۵۵ مگابایت بستهٔ سورس بود؛ کسی که کد را می‌خواند نباید بهای اسکرین‌شات هر فاز را بدهد |
>
> پس اگر در آرشیو سورس به پوشهٔ `screens/` برخوردید که خالی به‌نظر می‌رسد،
> تصویرهایش در آرشیو دوم‌اند. `dist/SHA256SUMS` هر دو را پوشش می‌دهد.
