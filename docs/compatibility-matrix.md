...
## قفل هم‌زمانی، مهاجرت و صفحه‌بندی (`alpha.14`)

| مورد | وضعیت | چه چیزی اندازه‌گیری شد |
|---|---|---|
| قفل خوش‌بینانه روی MariaDB واقعی | **Passed** | نسخه در `WHERE` و `row_version + 1` در همان `UPDATE`: از دو نویسندهٔ هم‌زمان یکی می‌نویسد و دیگری `stale_revision` می‌گیرد. `ConcurrentSaveTest` ۶ آزمون |
| دقت ثانیه در `DATETIME` | **Passed** | دو ذخیره که واقعاً در یک ثانیه افتادند: `updated_at` یکسان، شمارنده متفاوت. probe تکرار می‌شود تا دو ذخیره در یک ثانیه بیفتد، وگرنه نتیجه به لبهٔ ثانیه وابسته است |
| مهر اجرا روی ردیف، پس از قطع اجرا | **Passed** | فهرست (option) عمداً حذف شد؛ اجرا هنوز پیدا شد (`source=rows_only`) و rollback هم محصول و هم فروشگاه را برداشت |
| `INSERT IGNORE` و تفکیک صفر از null | **Passed** | `record()` سه جواب می‌دهد؛ جدولِ حذف‌شده `failed` می‌گیرد نه `already`. `DokanMigrationTest` |
| ارتقای درجا ۱۳ → ۱۶ | **Passed** | روی وردپرس یکبارمصرف با ووکامرس ۱۱٫۰٫۱ و دکان ۵٫۱٫۱: یک بازکردن صفحهٔ مدیر، سه ستون و یک جدول ساخته شد، `schema=16`، بدون خطا |
| `wp_max_upload_size()` کوچک‌تر از سقف ما | **Not Run** | روی این محیط سقف PHP از ۳ مگابایت بزرگ‌تر است، پس شاخهٔ «میزبان کوچک‌تر» با مقدار واقعی اجرا نشد؛ منطقش در `ProductImagePolicyTest` با مقدار تزریق‌شده آزموده شده |
| بازگشت `alpha.14 → alpha.13` روی سایت | **Not Run** | نصب دوباره در مجوز فعلی نیست. خطر شناخته‌شده و صریح نوشته شده: نسخهٔ قدیمی `row_version` را بالا نمی‌برد (`upgrade-and-rollback.md` ۷٫۴) |
| صفحه‌بندی صفحهٔ عمومی روی کاتالوگ واقعی | **Passed** | ۳۰ محصول منتشر و projected: ۲۴ + ۶، بدون تکرار، `pages+1` و `99999` هر دو ۴۰۴، `0`/`-3`/`abc`/`1.5` صفحهٔ یک با canonicalِ بی‌شماره |
| صفحهٔ عمومی زیر قالب سایت | **Not Run** | خروجی هنوز HTML مستقیم است؛ اتصال به قالب ساخته نشده |

## زیرساخت صف و رویداد (`alpha.13`)

| مورد | وضعیت | چه چیزی اندازه‌گیری شد |
|---|---|---|
| Action Scheduler (همراه ووکامرس ۱۱٫۰٫۱) | **Passed** | روی وردپرس یکبارمصرف، `binding=action_scheduler`، action تکرارشونده روی تقویم. اتصال روی `init` با اولویت ۲۰ ثبت می‌شود — پیش از آن انبار Action Scheduler ساخته نشده و فراخوانی `_doing_it_wrong` می‌دهد |
| WP-Cron بدون ووکامرس | **Not Run** | نصب بدون ووکامرس روی این محیط آزموده نشده؛ کد مسیرش را دارد و گزارش `wp_cron` می‌دهد |
| `DISABLE_WP_CRON` | **Not Run** | تشخیصش در کد هست (`wp_cron_disabled`) و صفحهٔ صف هشدارش را می‌دهد؛ روی سایتی با این ثابت آزموده نشده |
| `UPDATE` و تعداد سطر | **Passed** | MySQL سطرهای **تغییرکرده** را برمی‌گرداند و wpdb پرچم `CLIENT_FOUND_ROWS` را ست نمی‌کند — پس checkpoint بی‌تغییر صفر می‌دهد. `DbJobRepository::wrote()` در آن حالت مالکیت را از خودِ سطر می‌پرسد |
| ارسال بیرونی رویداد | **بسته، اندازه‌گیری‌شده** | ۸ رویداد، ۸تا `blocked` با دلیل `outbound_blocked`، payload و امضا دست‌نخورده. قفل `OutboundPolicy` با هیچ گزینه/ثابت/فیلتری باز نمی‌شود |
| افزونهٔ «دروازه» (Kamangir Smart Login) | **Not Run** | روی این محیط نصب نیست. مرجعش بررسی شد: هر ۱۶۲ فایل PHP آن SourceGuardian-encoded است و **هیچ hook یا فیلتری منتشر نمی‌کند** (`docs/otp-gateway-contract.md`). اجرایش روی سایت به افزونهٔ loader سازگار با نسخهٔ PHP همان سرور وابسته است |
| `<input type="date">` و حلقهٔ focus | **Passed، با نکته** | در کرومیوم چهار Tab می‌گیرد (روز، ماه، سال، دکمهٔ تقویم) و روی چهارمی `:focus` دیگر تطبیق نمی‌کند. `:focus-within` پوششش می‌دهد. روی فایرفاکس و سافاری `Not Run` |
...
## ۷. سطح عمومی فروشگاه و انتقال از دکان (`0.1.0-alpha.16`)

| مورد | وضعیت | مدرک / دلیل |
|---|---|---|
| صفحهٔ فروشگاه داخل قالب **کلاسیک** (Twenty Twenty-One) | **`Passed`** | ۱۲ بررسی در `store-surface/` بخش ۷؛ سربرگ، پاورقی، یک `<title>`، canonical و JSON-LD سالم |
| صفحهٔ فروشگاه به‌صورت **سند مستقل** | **`Passed`** | ۵۵ بررسی در `store-page/`، با حالت ست‌شدهٔ `standalone` |
| صفحهٔ فروشگاه داخل قالب **بلوکی** | `Not Run` | وردپرس یکبارمصرف قالب کلاسیک دارد. منطقِ «بدون `header.php`/`footer.php` ⇒ سند مستقل» آزمون واحد دارد، ولی روی قالب بلوکی واقعی اجرا نشده |
| دسترس‌پذیری (axe) صفحهٔ فروشگاهِ **داخل قالب** | `Not Run` | نتیجه به قالب هر سایت وابسته است؛ سوییت a11y ما سند مستقل را می‌سنجد |
| کش صفحهٔ عمومی و ابطالش | **`Passed`** | ۹ بررسی؛ `miss`→`hit`، بایت‌های یکسان، ابطال با تغییر محصول و با تغییر تصویر شاخص |
| sitemap فروشگاه‌ها، مسیریابی‌شده توسط هسته | **`Passed`** | نشانی‌ای که فهرست تبلیغ می‌کند `200 application/xml` می‌دهد و فقط فروشگاه تأییدشده را دارد |
| هدایت `/store/<slug>/` — دکان **فعال**، فروشگاه رفته | **`Passed`** | `301`؛ و فروشگاهی که دکان سرو می‌کند `200` و دست‌نخورده |
| هدایت `/store/<slug>/` — دکان **خاموش**، قاعده‌ها مانده | **`Passed`** | دکان واقعاً غیرفعال شد؛ پیش از اصلاح `200` صفحهٔ اول، پس از آن `301` |
| انتقال و بازگشت با حفظ تاریخچه | **`Passed`** | `cutover/` ۲۳ بررسی؛ digest دادهٔ دکان پیش/حین/پس یکسان، ردِ ممیزی باقی |
| ارتقا `alpha.14 → alpha.16` و بازگشت | **`Passed`** | `upgrade-alpha14/` ۳۵ بررسی، schema ۱۶→۱۷ بدون فعال‌سازی |
| ارتقای مستقیم از نسخه‌های پیش از `alpha.14` | `Not Run` | migrationها افزودنی و پشت‌سرهم‌اند، ولی این انتظار است نه اندازه‌گیری |
| مسیر کامل دسکتاپ و موبایل (ویدئو) | **`Passed`** | `walkthrough/`؛ دو اندازه، ۰ مشکل، از نسخهٔ نصب‌شده از ZIP |
| دکان **Pro** (پرسنل فروشنده) | `Not Run` | Lite پرسنل ندارد؛ خواننده روی Lite `[]` می‌دهد و گزارش می‌گوید «اینجا پیدا نشد»، نه «ندارد» |
