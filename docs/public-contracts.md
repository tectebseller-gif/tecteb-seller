...
جدول بالا **پیام‌ها** را توصیف می‌کند، نه شرط نوشتن ممیزی. ممیزی در همان
لحظه‌ای نوشته می‌شود که وردپرس ذخیره‌شدن option را تأیید می‌کند
(`update_option_tmc_settings` / `add_option_tmc_settings`) و به اجرای
`finalizeOutcome()` وابسته **نیست**: آن فیلتر فقط روی مسیر `options.php`
اجرا می‌شود، در حالی که هر نوشتن دیگری روی این option — کد افزونه‌ای دیگر،
WP-CLI، یا `update_option()` برنامه‌ای — هم باید ردپا بگذارد. یک نوشتن
موفق دقیقاً **یک** ردیف ممیزی می‌سازد، حتی وقتی وردپرس برای option تازه دو
بار sanitize را صدا می‌زند.

## ۵٫۲ نسخه schema: حالت‌های ممکن

| رابطه | خطای ثبت‌شده | وضعیت سلامت | رفتار migration |
|---|---|---|---|
| `stored == target` | ندارد | سالم | هیچ کاری |
| `stored == target` | دارد (**کهنه**) | سالم، با یادداشت جدا | رکورد هنگام فعال‌سازی دوباره پاک می‌شود |
| `stored < target` | ندارد | نیازمند اقدام | مهاجرت اجرا می‌شود (در `admin_init`، نه فقط activation) |
| `stored < target` | دارد | نیازمند اقدام | اجرای دوباره پس از cooldown |
| `stored > target` | — | **نیازمند اقدام** | **هرگز** اجرا نمی‌شود؛ ساختار پایین آورده نمی‌شود |

رکورد خطا وقتی `stored >= target` است **کهنه** شمرده می‌شود: اجرایی که آن را
نوشت با اجرای موفق بعدی جایگزین شده و تنها حذف رکورد ناموفق مانده است. fact
`last_error_stale` در بررسی `schema` همین را می‌گوید.
...
`tmc_db_version` و `tmc_db_last_error` **فقط** توسط اجرایی نوشته یا پاک
می‌شوند که در همان لحظه مالک `tmc_migration_lock` است؛ این شرط داخل خود
دستور SQL ارزیابی می‌شود، نه با یک `if` جداگانه پیش از نوشتن. جزئیات در
ADR-003 بند ۳٫۱.

نتیجه پاک‌سازی رکورد خطا چهار حالت متمایز دارد — نبودِ رکورد، حذف موفق،
نبودِ مالکیت، و شکست ذخیره‌سازی — و در `MigrationResult::$cleanup` گزارش
می‌شود. نبودِ رکورد شکست نیست؛ شکست ذخیره‌سازی هم موفقیت کامل نیست
(`isSuccess()` درست، `isFullySuccessful()` نادرست). ADR-003 بند ۳٫۲٫۱.
...
| `update_option_tmc_settings` / `add_option_tmc_settings` | action مصرفی | نقطه‌ای که وردپرس تأیید می‌کند مقدار ذخیره شد؛ ممیزی همان‌جا نوشته می‌شود، حتی اگر `options.php` اصلاً اجرا نشده باشد |
